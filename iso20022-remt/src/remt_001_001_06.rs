...
pub mod iso20022 {
	...
	impl GroupHeader122 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Some(ref cpy_ind_value) = self.cpy_ind { if let Err(e) = cpy_ind_value.validate() { return Err(e); } }
	...
	pub struct RemittanceAdviceV06 {
		...
		#[cfg_attr( feature = "derive_serde", serde(rename = "RmtInf") )]
	...
	impl RemittanceAdviceV06 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			for item in &self.rmt_inf { if let Err(e) = item.validate() { return Err(e); } }
	...
	impl RemittanceInformation23 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Err(e) = self.orgnl_pmt_inf.validate() { return Err(e); }
			Ok(())
		}
	}
