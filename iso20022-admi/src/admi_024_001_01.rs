...
pub mod iso20022 {
	...
	impl CorrespondenceNotification1 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if let Err(e) = self.sndr_ntfctn_id.validate() { return Err(e); }
			if let Err(e) = self.ntfctn_tp.validate() { return Err(e); }
			if let Some(ref ntfctn_sub_tp_value) = self.ntfctn_sub_tp { if let Err(e) = ntfctn_sub_tp_value.validate() { return Err(e); } }
			if let Some(ref ntfctn_nrrtv_vec) = self.ntfctn_nrrtv { for item in ntfctn_nrrtv_vec { if let Err(e) = item.validate() { return Err(e); } } }
			if let Some(ref nclsd_file_vec) = self.nclsd_file { for item in nclsd_file_vec { if let Err(e) = item.validate() { return Err(e); } } }
			if let Some(ref rltd_ntfctn_data_vec) = self.rltd_ntfctn_data { for item in rltd_ntfctn_data_vec { if let Err(e) = item.validate() { return Err(e); } } }
			Ok(())
		}
	}
	...
	impl ExternalNotificationSubType1Code {
		pub fn validate(&self) -> Result<(), ValidationError> {
			Ok(())
		}
	}
	...
	impl ExternalNotificationType1Code {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if self.external_notification_type1_code.chars().count() < 1 {
			return Err(ValidationError::new(1001, "external_notification_type1_code is shorter than the minimum length of 1".to_string()));
			}
			if self.external_notification_type1_code.chars().count() > 4 {
				return Err(ValidationError::new(1002, "external_notification_type1_code exceeds the maximum length of 4".to_string()));
			}
			Ok(())
		}
	}
	...
	impl RelatedNotificationData1 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if let Some(ref ntfctn_id_value) = self.ntfctn_id { if let Err(e) = ntfctn_id_value.validate() { return Err(e); } }
