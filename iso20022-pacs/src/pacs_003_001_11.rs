...
impl DirectDebitTransaction12 {
	pub fn validate(&self) -> bool {
		...
		if let Some(ref pre_ntfctn_id_value) = self.pre_ntfctn_id { if !pre_ntfctn_id_value.validate() { return false; } }
		return true
	}
}
...
impl DirectDebitTransactionInformation31 {
	pub fn validate(&self) -> bool {
		...
		if let Some(ref drct_dbt_tx_value) = self.drct_dbt_tx { if !drct_dbt_tx_value.validate() { return false; } }
...
impl FIToFICustomerDirectDebitV11 {
	pub fn validate(&self) -> bool {
		if !self.grp_hdr.validate() { return false }
		for item in &self.drct_dbt_tx_inf { if !item.validate() { return false; } }
		if let Some(ref splmtry_data_vec) = self.splmtry_data { for item in splmtry_data_vec { if !item.validate() { return false; } } }
		return true
	}
}
