...
impl CreditTransferTransaction62 {
	pub fn validate(&self) -> bool {
		...
		if let Some(ref undrlyg_allcn_vec) = self.undrlyg_allcn { for item in undrlyg_allcn_vec { if !item.validate() { return false; } } }
...
impl GroupHeader113 {
	pub fn validate(&self) -> bool {
		...
		if let Some(ref ttl_intr_bk_sttlm_amt_value) = self.ttl_intr_bk_sttlm_amt { if !ttl_intr_bk_sttlm_amt_value.validate() { return false; } }
...
impl References74Choice {
	pub fn validate(&self) -> bool {
		if let Some(ref scties_sttlm_tx_id_value) = self.scties_sttlm_tx_id { if !scties_sttlm_tx_id_value.validate() { return false; } }
		if let Some(ref intra_pos_mvmnt_id_value) = self.intra_pos_mvmnt_id { if !intra_pos_mvmnt_id_value.validate() { return false; } }
		if let Some(ref intra_bal_mvmnt_id_value) = self.intra_bal_mvmnt_id { if !intra_bal_mvmnt_id_value.validate() { return false; } }
		...
		if let Some(ref ctr_pty_mkt_infrstrctr_tx_id_value) = self.ctr_pty_mkt_infrstrctr_tx_id { if !ctr_pty_mkt_infrstrctr_tx_id_value.validate() { return false; } }
		if let Some(ref pool_id_value) = self.pool_id { if !pool_id_value.validate() { return false; } }
		if let Some(ref cmon_id_value) = self.cmon_id { if !cmon_id_value.validate() { return false; } }
		if let Some(ref trad_id_value) = self.trad_id { if !trad_id_value.validate() { return false; } }
		if let Some(ref othr_tx_id_value) = self.othr_tx_id { if !othr_tx_id_value.validate() { return false; } }
		return true
	}
}
...
impl TransactionAllocation1 {
	pub fn validate(&self) -> bool {
		...
		if !self.purp.validate() { return false }
		if !self.ref_attr.validate() { return false }
		if let Some(ref rltd_refs_vec) = self.rltd_refs { for item in rltd_refs_vec { if !item.validate() { return false; } } }
		return true
	}
}
