...
pub struct ExternalReversalReason1Code {
	#[serde(rename = "$value")]
	pub external_reversal_reason1_code: String,
}

impl ExternalReversalReason1Code {
	pub fn validate(&self) -> bool {
		if self.external_reversal_reason1_code.chars().count() < 1 {
			return false
		}
		if self.external_reversal_reason1_code.chars().count() > 4 {
			return false
		}
		return true
	}
...
impl GroupHeader127 {
	pub fn validate(&self) -> bool {
		...
		if let Some(ref ttl_rvsd_intr_bk_sttlm_amt_value) = self.ttl_rvsd_intr_bk_sttlm_amt { if !ttl_rvsd_intr_bk_sttlm_amt_value.validate() { return false; } }
...
impl OriginalGroupHeader20 {
	pub fn validate(&self) -> bool {
		...
		if let Some(ref rvsl_rsn_inf_vec) = self.rvsl_rsn_inf { for item in rvsl_rsn_inf_vec { if !item.validate() { return false; } } }
		return true
	}
}
...
impl PaymentTransaction149 {
	pub fn validate(&self) -> bool {
		if let Some(ref rvsl_id_value) = self.rvsl_id { if !rvsl_id_value.validate() { return false; } }
		if let Some(ref orgnl_grp_inf_value) = self.orgnl_grp_inf { if !orgnl_grp_inf_value.validate() { return false; } }
		...
		if !self.rvsd_intr_bk_sttlm_amt.validate() { return false }
		...
		if let Some(ref rvsd_instd_amt_value) = self.rvsd_instd_amt { if !rvsd_instd_amt_value.validate() { return false; } }
