...
pub mod iso20022 {
	...
	impl GroupHeader127 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Some(ref ttl_rvsd_intr_bk_sttlm_amt_value) = self.ttl_rvsd_intr_bk_sttlm_amt { if let Err(e) = ttl_rvsd_intr_bk_sttlm_amt_value.validate() { return Err(e); } }
	...
	impl PaymentTransaction149 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Err(e) = self.rvsd_intr_bk_sttlm_amt.validate() { return Err(e); }
			...
			if let Some(ref compstn_amt_value) = self.compstn_amt { if let Err(e) = compstn_amt_value.validate() { return Err(e); } }
