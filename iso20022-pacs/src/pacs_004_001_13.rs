...
impl GroupHeader123 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Some(ref ttl_rtrd_intr_bk_sttlm_amt_value) = self.ttl_rtrd_intr_bk_sttlm_amt { if let Err(e) = ttl_rtrd_intr_bk_sttlm_amt_value.validate() { return Err(e); } }
...
impl OriginalGroupHeader19 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Some(ref rtr_rsn_inf_vec) = self.rtr_rsn_inf { for item in rtr_rsn_inf_vec { if let Err(e) = item.validate() { return Err(e); } } }
		Ok(())
	}
}
...
impl PaymentTransaction159 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if let Some(ref rtr_id_value) = self.rtr_id { if let Err(e) = rtr_id_value.validate() { return Err(e); } }
		if let Some(ref orgnl_grp_inf_value) = self.orgnl_grp_inf { if let Err(e) = orgnl_grp_inf_value.validate() { return Err(e); } }
		...
		if let Some(ref rtrd_instd_amt_value) = self.rtrd_instd_amt { if let Err(e) = rtrd_instd_amt_value.validate() { return Err(e); } }
		...
		if let Some(ref instg_agt_value) = self.instg_agt { if let Err(e) = instg_agt_value.validate() { return Err(e); } }
		if let Some(ref instd_agt_value) = self.instd_agt { if let Err(e) = instd_agt_value.validate() { return Err(e); } }
		if let Some(ref rtr_chain_value) = self.rtr_chain { if let Err(e) = rtr_chain_value.validate() { return Err(e); } }
		if let Some(ref rtr_rsn_inf_vec) = self.rtr_rsn_inf { for item in rtr_rsn_inf_vec { if let Err(e) = item.validate() { return Err(e); } } }
