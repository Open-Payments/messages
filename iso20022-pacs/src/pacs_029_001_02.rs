...
pub mod iso20022 {
	...
	impl GroupHeader104 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Err(e) = self.nb_of_sttlm_reqs.validate() { return Err(e); }
	...
	impl MovementRecord2 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if let Err(e) = self.id.validate() { return Err(e); }
			if let Err(e) = self.amt.validate() { return Err(e); }
			if let Some(ref sttlm_agt_value) = self.sttlm_agt { if let Err(e) = sttlm_agt_value.validate() { return Err(e); } }
			if let Some(ref sttlm_agt_acct_value) = self.sttlm_agt_acct { if let Err(e) = sttlm_agt_acct_value.validate() { return Err(e); } }
			if let Some(ref ptcpt_value) = self.ptcpt { if let Err(e) = ptcpt_value.validate() { return Err(e); } }
			if let Some(ref ptcpt_acct_value) = self.ptcpt_acct { if let Err(e) = ptcpt_acct_value.validate() { return Err(e); } }
	...
	impl MultilateralSettlementRequest3 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if let Err(e) = self.instr_id.validate() { return Err(e); }
			...
			for item in &self.mvmnt_rcrd { if let Err(e) = item.validate() { return Err(e); } }
			Ok(())
		}
	}
	...
	pub struct MultilateralSettlementRequestV02 {
		...
		#[cfg_attr( feature = "derive_serde", serde(rename = "SttlmReq") )]
	...
	impl MultilateralSettlementRequestV02 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			for item in &self.sttlm_req { if let Err(e) = item.validate() { return Err(e); } }
			if let Some(ref splmtry_data_value) = self.splmtry_data { if let Err(e) = splmtry_data_value.validate() { return Err(e); } }
			Ok(())
		}
	}
