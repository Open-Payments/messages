...
impl BusinessApplicationHeader8 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Err(e) = self.biz_msg_idr.validate() { return Err(e); }
		if let Err(e) = self.msg_def_idr.validate() { return Err(e); }
		if let Some(ref biz_svc_value) = self.biz_svc { if let Err(e) = biz_svc_value.validate() { return Err(e); } }
		if let Some(ref mkt_prctc_value) = self.mkt_prctc { if let Err(e) = mkt_prctc_value.validate() { return Err(e); } }
...
impl ImplementationSpecification1 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if let Err(e) = self.regy.validate() { return Err(e); }
		if let Err(e) = self.id.validate() { return Err(e); }
		Ok(())
	}
}
