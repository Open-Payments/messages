...
pub struct ExternalMandateSuspensionReason1Code {
	#[serde(rename = "$value")]
	pub external_mandate_suspension_reason1_code: String,
}

impl ExternalMandateSuspensionReason1Code {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if self.external_mandate_suspension_reason1_code.chars().count() < 1 {
			return Err(ValidationError::new(1001, "external_mandate_suspension_reason1_code is shorter than the minimum length of 1".to_string()));
		}
		if self.external_mandate_suspension_reason1_code.chars().count() > 4 {
			return Err(ValidationError::new(1002, "external_mandate_suspension_reason1_code exceeds the maximum length of 4".to_string()));
		}
		Ok(())
	}
}
...
impl Mandate20 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if let Err(e) = self.mndt_id.validate() { return Err(e); }
...
pub struct MandateSuspensionRequestV04 {
	...
	#[serde(rename = "UndrlygSspnsnDtls")]
	pub undrlyg_sspnsn_dtls: Vec<MandateSuspension4>,
	#[serde(rename = "SplmtryData", skip_serializing_if = "Option::is_none")]
	pub splmtry_data: Option<Vec<SupplementaryData1>>,
}

impl MandateSuspensionRequestV04 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if let Err(e) = self.grp_hdr.validate() { return Err(e); }
		for item in &self.undrlyg_sspnsn_dtls { if let Err(e) = item.validate() { return Err(e); } }
		if let Some(ref splmtry_data_vec) = self.splmtry_data { for item in splmtry_data_vec { if let Err(e) = item.validate() { return Err(e); } } }
		Ok(())
	}
}
