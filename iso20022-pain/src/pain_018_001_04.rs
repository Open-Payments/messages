...
impl MandateSuspension4 {
	pub fn validate(&self) -> bool {
		if !self.sspnsn_req_id.validate() { return false }
		if let Some(ref orgnl_msg_inf_value) = self.orgnl_msg_inf { if !orgnl_msg_inf_value.validate() { return false; } }
		if !self.sspnsn_rsn.validate() { return false }
		if !self.orgnl_mndt.validate() { return false }
...
impl MandateSuspensionRequestV04 {
	pub fn validate(&self) -> bool {
		...
		for item in &self.undrlyg_sspnsn_dtls { if !item.validate() { return false; } }
		if let Some(ref splmtry_data_vec) = self.splmtry_data { for item in splmtry_data_vec { if !item.validate() { return false; } } }
		return true
	}
}
