...
pub mod iso20022 {
	...
	pub struct CustomerPaymentReversalV12 {
		...
		#[cfg_attr( feature = "derive_serde", serde(rename = "OrgnlPmtInfAndRvsl", skip_serializing_if = "Option::is_none") )]
	...
	impl CustomerPaymentReversalV12 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Some(ref orgnl_pmt_inf_and_rvsl_vec) = self.orgnl_pmt_inf_and_rvsl { for item in orgnl_pmt_inf_and_rvsl_vec { if let Err(e) = item.validate() { return Err(e); } } }
	...
	impl ExternalReversalReason1Code {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			return Err(ValidationError::new(1001, "external_reversal_reason1_code is shorter than the minimum length of 1".to_string()));
			}
			if self.external_reversal_reason1_code.chars().count() > 4 {
				return Err(ValidationError::new(1002, "external_reversal_reason1_code exceeds the maximum length of 4".to_string()));
			}
			Ok(())
		}
	}
	...
	impl OriginalGroupHeader20 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Some(ref rvsl_rsn_inf_vec) = self.rvsl_rsn_inf { for item in rvsl_rsn_inf_vec { if let Err(e) = item.validate() { return Err(e); } } }
			Ok(())
		}
	}
	...
	impl OriginalPaymentInstruction50 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if let Some(ref rvsl_pmt_inf_id_value) = self.rvsl_pmt_inf_id { if let Err(e) = rvsl_pmt_inf_id_value.validate() { return Err(e); } }
	...
	impl PaymentTransaction156 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if let Some(ref rvsl_id_value) = self.rvsl_id { if let Err(e) = rvsl_id_value.validate() { return Err(e); } }
			...
			if let Some(ref rvsd_instd_amt_value) = self.rvsd_instd_amt { if let Err(e) = rvsd_instd_amt_value.validate() { return Err(e); } }
