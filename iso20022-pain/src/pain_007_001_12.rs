...
impl CustomerPaymentReversalV12 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Some(ref vec) = self.orgnl_pmt_inf_and_rvsl { for item in vec { if let Err(e) = item.validate() { return Err(e); } } }
...
impl OriginalGroupHeader20 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Some(ref vec) = self.rvsl_rsn_inf { for item in vec { if let Err(e) = item.validate() { return Err(e); } } }
		Ok(())
	}
}
...
impl PaymentTransaction156 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Some(ref val) = self.rvsd_instd_amt { if let Err(e) = val.validate() { return Err(e); } }
