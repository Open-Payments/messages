...
pub mod iso20022 {
	...
	impl PaymentTransaction160 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Some(ref trckr_data_value) = self.trckr_data { if let Err(e) = trckr_data_value.validate() { return Err(e); } }
	...
	impl TrackerRecord5 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Some(ref chrgs_amt_value) = self.chrgs_amt { if let Err(e) = chrgs_amt_value.validate() { return Err(e); } }
			if let Some(ref xchg_rate_data_value) = self.xchg_rate_data { if let Err(e) = xchg_rate_data_value.validate() { return Err(e); } }
			Ok(())
		}
	}
