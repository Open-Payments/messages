...
impl PaymentTransaction160 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Some(ref val) = self.trckr_data { if let Err(e) = val.validate() { return Err(e); } }
...
impl TrackerData7 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if let Err(e) = self.confd_dt.validate() { return Err(e); }
		if let Err(e) = self.confd_amt.validate() { return Err(e); }
		for item in &self.trckr_rcrd { if let Err(e) = item.validate() { return Err(e); } }
		Ok(())
	}
}
...
impl TrackerRecord5 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Some(ref val) = self.chrgs_amt { if let Err(e) = val.validate() { return Err(e); } }
		if let Some(ref val) = self.xchg_rate_data { if let Err(e) = val.validate() { return Err(e); } }
		Ok(())
	}
}
