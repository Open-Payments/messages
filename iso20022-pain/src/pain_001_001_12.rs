...
impl CreditTransferTransaction61 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Some(ref val) = self.instr_for_dbtr_agt { if let Err(e) = val.validate() { return Err(e); } }
...
impl GroupHeader114 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Some(ref val) = self.initn_src { if let Err(e) = val.validate() { return Err(e); } }
		Ok(())
	}
}
