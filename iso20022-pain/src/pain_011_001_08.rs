...
// MandateCancellationRequestV08 ...
#[derive(Debug, Default, PartialEq, Clone, Serialize, Deserialize)]
pub struct MandateCancellationRequestV08 {
	#[serde(rename = "GrpHdr")]
	pub grp_hdr: GroupHeader110,
	#[serde(rename = "UndrlygCxlDtls")]
	pub undrlyg_cxl_dtls: Vec<MandateCancellation8>,
	#[serde(rename = "SplmtryData", skip_serializing_if = "Option::is_none")]
	pub splmtry_data: Option<Vec<SupplementaryData1>>,
}

impl MandateCancellationRequestV08 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if let Err(e) = self.grp_hdr.validate() { return Err(e); }
		for item in &self.undrlyg_cxl_dtls { if let Err(e) = item.validate() { return Err(e); } }
		if let Some(ref splmtry_data_vec) = self.splmtry_data { for item in splmtry_data_vec { if let Err(e) = item.validate() { return Err(e); } } }
		Ok(())
	}
}
