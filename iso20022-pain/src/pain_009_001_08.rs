...
pub struct Mandate23 {
	...
	pub mndt_id: Option<Vec<Max35Text>>,
...
impl Mandate23 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if let Some(ref mndt_id_vec) = self.mndt_id { for item in mndt_id_vec { if let Err(e) = item.validate() { return Err(e); } } }
		if let Err(e) = self.mndt_req_id.validate() { return Err(e); }
...
impl MandateInitiationRequestV08 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if let Err(e) = self.grp_hdr.validate() { return Err(e); }
		for item in &self.mndt { if let Err(e) = item.validate() { return Err(e); } }
		if let Some(ref splmtry_data_vec) = self.splmtry_data { for item in splmtry_data_vec { if let Err(e) = item.validate() { return Err(e); } } }
		Ok(())
	}
}
