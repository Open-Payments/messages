...
pub mod iso20022 {
	...
	impl DirectDebitTransaction12 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Some(ref pre_ntfctn_id_value) = self.pre_ntfctn_id { if let Err(e) = pre_ntfctn_id_value.validate() { return Err(e); } }
			Ok(())
		}
	}
	...
	impl DirectDebitTransactionInformation32 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Some(ref drct_dbt_tx_value) = self.drct_dbt_tx { if let Err(e) = drct_dbt_tx_value.validate() { return Err(e); } }
			...
			if let Some(ref instr_for_cdtr_agt_value) = self.instr_for_cdtr_agt { if let Err(e) = instr_for_cdtr_agt_value.validate() { return Err(e); } }
