...
pub mod iso20022 {
	...
	impl ExternalMandateStatus1Code {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if self.external_mandate_status1_code.chars().count() < 1 {
			return Err(ValidationError::new(1001, "external_mandate_status1_code is shorter than the minimum length of 1".to_string()));
			}
			if self.external_mandate_status1_code.chars().count() > 4 {
				return Err(ValidationError::new(1002, "external_mandate_status1_code exceeds the maximum length of 4".to_string()));
			}
			Ok(())
		}
	}
	...
	impl MandateCopy4 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Some(ref mndt_sts_value) = self.mndt_sts { if let Err(e) = mndt_sts_value.validate() { return Err(e); } }
	...
	pub struct MandateCopyRequestV04 {
		...
		#[cfg_attr( feature = "derive_serde", serde(rename = "UndrlygCpyReqDtls") )]
		pub undrlyg_cpy_req_dtls: Vec<MandateCopy4>,
		#[cfg_attr( feature = "derive_serde", serde(rename = "SplmtryData", skip_serializing_if = "Option::is_none") )]
		pub splmtry_data: Option<Vec<SupplementaryData1>>,
	}
	
	impl MandateCopyRequestV04 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if let Err(e) = self.grp_hdr.validate() { return Err(e); }
			for item in &self.undrlyg_cpy_req_dtls { if let Err(e) = item.validate() { return Err(e); } }
			if let Some(ref splmtry_data_vec) = self.splmtry_data { for item in splmtry_data_vec { if let Err(e) = item.validate() { return Err(e); } } }
			Ok(())
		}
	}
