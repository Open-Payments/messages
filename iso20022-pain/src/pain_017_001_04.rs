...
impl MandateCopy4 {
	pub fn validate(&self) -> bool {
		...
		if let Some(ref mndt_sts_value) = self.mndt_sts { if !mndt_sts_value.validate() { return false; } }
...
impl MandateCopyRequestV04 {
	pub fn validate(&self) -> bool {
		...
		for item in &self.undrlyg_cpy_req_dtls { if !item.validate() { return false; } }
		if let Some(ref splmtry_data_vec) = self.splmtry_data { for item in splmtry_data_vec { if !item.validate() { return false; } } }
		return true
	}
}
