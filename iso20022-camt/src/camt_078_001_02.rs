...
pub mod iso20022 {
	...
	impl IntraBalanceQueryCriteria11 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if let Some(ref refs_vec) = self.refs { for item in refs_vec { if let Err(e) = item.validate() { return Err(e); } } }
			if let Some(ref sts_value) = self.sts { if let Err(e) = sts_value.validate() { return Err(e); } }
			if let Some(ref csh_acct_vec) = self.csh_acct { for item in csh_acct_vec { if let Err(e) = item.validate() { return Err(e); } } }
			if let Some(ref csh_acct_ownr_vec) = self.csh_acct_ownr { for item in csh_acct_ownr_vec { if let Err(e) = item.validate() { return Err(e); } } }
			if let Some(ref csh_acct_svcr_value) = self.csh_acct_svcr { if let Err(e) = csh_acct_svcr_value.validate() { return Err(e); } }
			if let Some(ref bal_tp_vec) = self.bal_tp { for item in bal_tp_vec { if let Err(e) = item.validate() { return Err(e); } } }
			if let Some(ref csh_sub_bal_id_vec) = self.csh_sub_bal_id { for item in csh_sub_bal_id_vec { if let Err(e) = item.validate() { return Err(e); } } }
			if let Some(ref sttlm_amt_value) = self.sttlm_amt { if let Err(e) = sttlm_amt_value.validate() { return Err(e); } }
			if let Some(ref sttld_amt_value) = self.sttld_amt { if let Err(e) = sttld_amt_value.validate() { return Err(e); } }
			if let Some(ref sttlm_ccy_vec) = self.sttlm_ccy { for item in sttlm_ccy_vec { if let Err(e) = item.validate() { return Err(e); } } }
			if let Some(ref intndd_sttlm_dt_value) = self.intndd_sttlm_dt { if let Err(e) = intndd_sttlm_dt_value.validate() { return Err(e); } }
			if let Some(ref fctv_sttlm_dt_value) = self.fctv_sttlm_dt { if let Err(e) = fctv_sttlm_dt_value.validate() { return Err(e); } }
			if let Some(ref prty_vec) = self.prty { for item in prty_vec { if let Err(e) = item.validate() { return Err(e); } } }
			if let Some(ref msg_orgtr_vec) = self.msg_orgtr { for item in msg_orgtr_vec { if let Err(e) = item.validate() { return Err(e); } } }
			if let Some(ref cre_dt_tm_value) = self.cre_dt_tm { if let Err(e) = cre_dt_tm_value.validate() { return Err(e); } }
			Ok(())
		}
	}
	...
	impl IntraBalanceStatusType2 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Some(ref sttld_value) = self.sttld { if let Err(e) = sttld_value.validate() { return Err(e); } }
			Ok(())
		}
	}
	...
	impl IntraBalanceType3 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if let Some(ref bal_fr_value) = self.bal_fr { if let Err(e) = bal_fr_value.validate() { return Err(e); } }
			if let Some(ref bal_to_value) = self.bal_to { if let Err(e) = bal_to_value.validate() { return Err(e); } }
			Ok(())
		}
	}
