...
pub mod iso20022 {
	...
	impl Cheque18 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Err(e) = self.chq_cxl_or_stop_sts.validate() { return Err(e); }
			Ok(())
		}
	}
	
	
	// ChequeCancellationOrStopReportV02 ...
	#[cfg_attr(feature = "derive_debug", derive(Debug))]
	#[cfg_attr(feature = "derive_clone", derive(Clone))]
	#[cfg_attr(feature = "derive_partial_eq", derive(PartialEq))]
	#[cfg_attr(feature = "derive_default", derive(Default))]
	...
	impl ChequeCancellationOrStopReportV02 {
		pub fn validate(&self) -> Result<(), ValidationError> {
	...
	pub struct ExternalChequeCancellationStatus1Code {
		#[cfg_attr( feature = "derive_serde", serde(rename = "$value") )]
		pub external_cheque_cancellation_status1_code: String,
	}
	
	impl ExternalChequeCancellationStatus1Code {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if self.external_cheque_cancellation_status1_code.chars().count() < 1 {
			return Err(ValidationError::new(1001, "external_cheque_cancellation_status1_code is shorter than the minimum length of 1".to_string()));
			}
			if self.external_cheque_cancellation_status1_code.chars().count() > 4 {
				return Err(ValidationError::new(1002, "external_cheque_cancellation_status1_code exceeds the maximum length of 4".to_string()));
			}
			Ok(())
		}
	}
