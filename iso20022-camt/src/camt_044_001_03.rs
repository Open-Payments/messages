...
impl Fund2 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Some(ref val) = self.trad_dt_tm { if let Err(e) = val.validate() { return Err(e); } }
		if let Some(ref val) = self.prvs_trad_dt_tm { if let Err(e) = val.validate() { return Err(e); } }
		if let Some(ref val) = self.ttl_nav { if let Err(e) = val.validate() { return Err(e); } }
		...
		if let Some(ref vec) = self.csh_in_fcst_dtls { for item in vec { if let Err(e) = item.validate() { return Err(e); } } }
		if let Some(ref vec) = self.csh_out_fcst_dtls { for item in vec { if let Err(e) = item.validate() { return Err(e); } } }
		if let Some(ref vec) = self.net_csh_fcst_dtls { for item in vec { if let Err(e) = item.validate() { return Err(e); } } }
		Ok(())
	}
}
...
impl FundConfirmedCashForecastReport3 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if let Some(ref vec) = self.fnd_or_sub_fnd_dtls { for item in vec { if let Err(e) = item.validate() { return Err(e); } } }
		if let Some(ref vec) = self.fnd_csh_fcst_dtls { for item in vec { if let Err(e) = item.validate() { return Err(e); } } }
...
impl FundConfirmedCashForecastReportCancellationV03 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Some(ref val) = self.csh_fcst_rpt_to_be_canc { if let Err(e) = val.validate() { return Err(e); } }
		Ok(())
	}
}
