...
pub mod iso20022 {
	...
	impl GetReservationV08 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Some(ref val) = self.rsvatn_qry_def { if let Err(e) = val.validate() { return Err(e); } }
	...
	impl ReservationQuery6 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Some(ref val) = self.rsvatn_crit { if let Err(e) = val.validate() { return Err(e); } }
			Ok(())
		}
	}
	...
	impl ReservationSearchCriteria6 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Some(ref vec) = self.dflt_rsvatn_tp { for item in vec { if let Err(e) = item.validate() { return Err(e); } } }
			if let Some(ref vec) = self.cur_rsvatn_tp { for item in vec { if let Err(e) = item.validate() { return Err(e); } } }
