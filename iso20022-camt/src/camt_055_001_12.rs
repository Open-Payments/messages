...
pub mod iso20022 {
	...
	impl CustomerPaymentCancellationRequestV12 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Some(ref val) = self.ctrl_data { if let Err(e) = val.validate() { return Err(e); } }
	...
	impl OriginalGroupHeader21 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Some(ref vec) = self.cxl_rsn_inf { for item in vec { if let Err(e) = item.validate() { return Err(e); } } }
			Ok(())
		}
	}
	...
	impl UnderlyingTransaction33 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if let Some(ref val) = self.orgnl_grp_inf_and_cxl { if let Err(e) = val.validate() { return Err(e); } }
			if let Some(ref vec) = self.orgnl_pmt_inf_and_cxl { for item in vec { if let Err(e) = item.validate() { return Err(e); } } }
			Ok(())
		}
	}
