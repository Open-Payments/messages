...
impl AdditionalParameters16 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if let Some(ref prtl_sttlm_value) = self.prtl_sttlm { if let Err(e) = prtl_sttlm_value.validate() { return Err(e); } }
		if let Some(ref prvs_prtl_conf_id_value) = self.prvs_prtl_conf_id { if let Err(e) = prvs_prtl_conf_id_value.validate() { return Err(e); } }
...
impl IntraBalance6 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Some(ref prevsly_sttld_amt_value) = self.prevsly_sttld_amt { if let Err(e) = prevsly_sttld_amt_value.validate() { return Err(e); } }
		if let Some(ref rmng_sttlm_amt_value) = self.rmng_sttlm_amt { if let Err(e) = rmng_sttlm_amt_value.validate() { return Err(e); } }
...
// IntraBalanceMovementConfirmationV02 ...
#[derive(Debug, Default, PartialEq, Clone, Serialize, Deserialize)]
...
impl IntraBalanceMovementConfirmationV02 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if let Some(ref id_value) = self.id { if let Err(e) = id_value.validate() { return Err(e); } }
		...
		if let Err(e) = self.intra_bal.validate() { return Err(e); }
