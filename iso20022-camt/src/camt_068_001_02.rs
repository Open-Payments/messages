...
impl AdditionalParameters16 {
	pub fn validate(&self) -> bool {
		if let Some(ref prtl_sttlm_value) = self.prtl_sttlm { if !prtl_sttlm_value.validate() { return false; } }
		if let Some(ref prvs_prtl_conf_id_value) = self.prvs_prtl_conf_id { if !prvs_prtl_conf_id_value.validate() { return false; } }
