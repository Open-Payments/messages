...
pub mod iso20022 {
	...
	impl MissingOrIncorrectData1 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if let Some(ref vec) = self.mssng_inf { for item in vec { if let Err(e) = item.validate() { return Err(e); } } }
			if let Some(ref vec) = self.incrrct_inf { for item in vec { if let Err(e) = item.validate() { return Err(e); } } }
			Ok(())
		}
	}
	...
	impl UnableToApplyJustification4Choice {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if let Some(ref val) = self.mssng_or_incrrct_inf { if let Err(e) = val.validate() { return Err(e); } }
			Ok(())
		}
	}
