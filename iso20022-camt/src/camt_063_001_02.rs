...
impl AcknowledgementDetails1Choice {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if let Some(ref pay_in_schdl_ref_value) = self.pay_in_schdl_ref { if let Err(e) = pay_in_schdl_ref_value.validate() { return Err(e); } }
		if let Some(ref pay_in_call_ref_value) = self.pay_in_call_ref { if let Err(e) = pay_in_call_ref_value.validate() { return Err(e); } }
		Ok(())
	}
}
