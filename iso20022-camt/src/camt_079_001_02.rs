...
pub mod iso20022 {
	...
	impl IntraBalanceMovement6 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Some(ref val) = self.prevsly_sttld_amt { if let Err(e) = val.validate() { return Err(e); } }
			if let Some(ref val) = self.rmng_sttlm_amt { if let Err(e) = val.validate() { return Err(e); } }
	...
	impl IntraBalanceMovement7 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Some(ref val) = self.mvmnt_dtls { if let Err(e) = val.validate() { return Err(e); } }
			Ok(())
		}
	}
	...
	impl IntraBalanceMovementQueryResponseV02 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Some(ref val) = self.rpt_or_err { if let Err(e) = val.validate() { return Err(e); } }
			Ok(())
		}
	}
