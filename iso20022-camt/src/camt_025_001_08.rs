...
pub mod iso20022 {
	...
	impl Receipt6 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Some(ref val) = self.orgnl_pmt_id { if let Err(e) = val.validate() { return Err(e); } }
			if let Some(ref vec) = self.req_hdlg { for item in vec { if let Err(e) = item.validate() { return Err(e); } } }
			Ok(())
		}
	}
