...
impl OriginalMessageAndIssuer1 {
	pub fn validate(&self) -> bool {
		...
		if let Some(ref orgtr_nm_value) = self.orgtr_nm { if !orgtr_nm_value.validate() { return false; } }
		return true
	}
}
...
impl Receipt6 {
	pub fn validate(&self) -> bool {
		...
		if let Some(ref orgnl_pmt_id_value) = self.orgnl_pmt_id { if !orgnl_pmt_id_value.validate() { return false; } }
		if let Some(ref req_hdlg_vec) = self.req_hdlg { for item in req_hdlg_vec { if !item.validate() { return false; } } }
		return true
	}
}
...
impl ReceiptV08 {
	pub fn validate(&self) -> bool {
		...
		for item in &self.rct_dtls { if !item.validate() { return false; } }
