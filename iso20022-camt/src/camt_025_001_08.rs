...
#[derive(Debug, Default, PartialEq, Clone, Serialize, Deserialize)]
#[serde(transparent)]
pub struct ExternalRequestStatus1Code {
	#[serde(rename = "$value")]
	pub external_request_status1_code: String,
}

impl ExternalRequestStatus1Code {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if self.external_request_status1_code.chars().count() < 1 {
			return Err(ValidationError::new(1001, "external_request_status1_code is shorter than the minimum length of 1".to_string()));
		}
		if self.external_request_status1_code.chars().count() > 4 {
			return Err(ValidationError::new(1002, "external_request_status1_code exceeds the maximum length of 4".to_string()));
		}
		Ok(())
	}
}
...
impl OriginalMessageAndIssuer1 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Some(ref orgtr_nm_value) = self.orgtr_nm { if let Err(e) = orgtr_nm_value.validate() { return Err(e); } }
		Ok(())
	}
}
...
pub struct Receipt6 {
	...
	#[serde(rename = "ReqHdlg", skip_serializing_if = "Option::is_none")]
...
impl Receipt6 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Some(ref orgnl_pmt_id_value) = self.orgnl_pmt_id { if let Err(e) = orgnl_pmt_id_value.validate() { return Err(e); } }
		if let Some(ref req_hdlg_vec) = self.req_hdlg { for item in req_hdlg_vec { if let Err(e) = item.validate() { return Err(e); } } }
		Ok(())
	}
}
...
impl ReceiptV08 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		for item in &self.rct_dtls { if let Err(e) = item.validate() { return Err(e); } }
