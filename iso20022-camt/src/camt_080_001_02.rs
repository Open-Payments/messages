...
pub struct IntraBalanceQueryCriteria12 {
	#[serde(rename = "ModReqId", skip_serializing_if = "Option::is_none")]
	pub mod_req_id: Option<Vec<Max35Text>>,
...
impl IntraBalanceQueryCriteria12 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if let Some(ref mod_req_id_vec) = self.mod_req_id { for item in mod_req_id_vec { if let Err(e) = item.validate() { return Err(e); } } }
