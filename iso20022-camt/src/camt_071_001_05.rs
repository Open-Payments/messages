...
impl DeleteStandingOrderV05 {
	pub fn validate(&self) -> bool {
		...
		if !self.stg_ordr_dtls.validate() { return false }
...
impl StandingOrderOrAll4Choice {
	pub fn validate(&self) -> bool {
		...
		if let Some(ref all_stg_ordrs_vec) = self.all_stg_ordrs { for item in all_stg_ordrs_vec { if !item.validate() { return false; } } }
		return true
	}
}
