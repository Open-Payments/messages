...
pub mod iso20022 {
	...
	pub struct DeleteStandingOrderV05 {
		...
		#[cfg_attr( feature = "derive_serde", serde(rename = "StgOrdrDtls") )]
		pub stg_ordr_dtls: StandingOrderOrAll4Choice,
		#[cfg_attr( feature = "derive_serde", serde(rename = "SplmtryData", skip_serializing_if = "Option::is_none") )]
		pub splmtry_data: Option<Vec<SupplementaryData1>>,
	}
	
	impl DeleteStandingOrderV05 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if let Err(e) = self.msg_hdr.validate() { return Err(e); }
			if let Err(e) = self.stg_ordr_dtls.validate() { return Err(e); }
	...
	impl StandingOrderOrAll4Choice {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Some(ref all_stg_ordrs_vec) = self.all_stg_ordrs { for item in all_stg_ordrs_vec { if let Err(e) = item.validate() { return Err(e); } } }
			Ok(())
		}
	}
