...
impl CurrentAndDefaultReservation6 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if let Some(ref cur_rsvatn_vec) = self.cur_rsvatn { for item in cur_rsvatn_vec { if let Err(e) = item.validate() { return Err(e); } } }
		if let Some(ref dflt_rsvatn_vec) = self.dflt_rsvatn { for item in dflt_rsvatn_vec { if let Err(e) = item.validate() { return Err(e); } } }
		Ok(())
	}
}
...
impl ReservationOrError9Choice {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if let Some(ref rsvatn_value) = self.rsvatn { if let Err(e) = rsvatn_value.validate() { return Err(e); } }
