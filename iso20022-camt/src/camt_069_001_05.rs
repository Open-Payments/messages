...
pub mod iso20022 {
	...
	impl GetStandingOrderV05 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Some(ref val) = self.stg_ordr_qry_def { if let Err(e) = val.validate() { return Err(e); } }
	...
	impl StandingOrderQuery5 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Some(ref val) = self.stg_ordr_crit { if let Err(e) = val.validate() { return Err(e); } }
			Ok(())
		}
	}
