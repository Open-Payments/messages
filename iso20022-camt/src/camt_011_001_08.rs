...
impl Amount4Choice {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if let Some(ref incr_amt_value) = self.incr_amt { if let Err(e) = incr_amt_value.validate() { return Err(e); } }
		if let Some(ref dcr_amt_value) = self.dcr_amt { if let Err(e) = dcr_amt_value.validate() { return Err(e); } }
		Ok(())
	}
}
...
impl LimitIdentification3Choice {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Some(ref all_cur_value) = self.all_cur { if let Err(e) = all_cur_value.validate() { return Err(e); } }
		if let Some(ref all_dflt_value) = self.all_dflt { if let Err(e) = all_dflt_value.validate() { return Err(e); } }
		Ok(())
	}
}
...
impl LimitIdentification8 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Some(ref bil_lmt_ctr_pty_id_value) = self.bil_lmt_ctr_pty_id { if let Err(e) = bil_lmt_ctr_pty_id_value.validate() { return Err(e); } }
...
impl LimitStructure5 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if let Err(e) = self.lmt_id.validate() { return Err(e); }
		if let Err(e) = self.new_lmt_val_set.validate() { return Err(e); }
		if let Some(ref od_lmt_val_set_value) = self.od_lmt_val_set { if let Err(e) = od_lmt_val_set_value.validate() { return Err(e); } }
		if let Some(ref lmt_val_amdmnt_value) = self.lmt_val_amdmnt { if let Err(e) = lmt_val_amdmnt_value.validate() { return Err(e); } }
		Ok(())
	}
}
...
impl ModifyLimitV08 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Some(ref lmt_dtls_vec) = self.lmt_dtls { for item in lmt_dtls_vec { if let Err(e) = item.validate() { return Err(e); } } }
