...
pub mod iso20022 {
	...
	impl IntraBalanceMovementInstructionV02 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Some(ref nb_counts_value) = self.nb_counts { if let Err(e) = nb_counts_value.validate() { return Err(e); } }
	...
	impl NumberCount1Choice {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if let Some(ref cur_instr_nb_value) = self.cur_instr_nb { if let Err(e) = cur_instr_nb_value.validate() { return Err(e); } }
			if let Some(ref ttl_nb_value) = self.ttl_nb { if let Err(e) = ttl_nb_value.validate() { return Err(e); } }
			Ok(())
		}
	}
	...
	impl TotalNumber1 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if let Err(e) = self.cur_instr_nb.validate() { return Err(e); }
			if let Err(e) = self.ttl_of_lkd_instrs.validate() { return Err(e); }
			Ok(())
		}
	}
