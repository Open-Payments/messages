...
pub struct Cheque17 {
	...
	#[serde(rename = "InstrForChqAgt", skip_serializing_if = "Option::is_none")]
...
impl Cheque17 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Err(e) = self.chq_nb.validate() { return Err(e); }
		if let Err(e) = self.amt.validate() { return Err(e); }
		if let Some(ref val_dt_value) = self.val_dt { if let Err(e) = val_dt_value.validate() { return Err(e); } }
		if let Some(ref pyer_value) = self.pyer { if let Err(e) = pyer_value.validate() { return Err(e); } }
		if let Some(ref pyer_acct_value) = self.pyer_acct { if let Err(e) = pyer_acct_value.validate() { return Err(e); } }
		if let Some(ref drwr_agt_value) = self.drwr_agt { if let Err(e) = drwr_agt_value.validate() { return Err(e); } }
		if let Some(ref drwr_agt_acct_value) = self.drwr_agt_acct { if let Err(e) = drwr_agt_acct_value.validate() { return Err(e); } }
		if let Err(e) = self.pyee.validate() { return Err(e); }
		if let Some(ref pyee_acct_value) = self.pyee_acct { if let Err(e) = pyee_acct_value.validate() { return Err(e); } }
		if let Some(ref instr_for_chq_agt_vec) = self.instr_for_chq_agt { for item in instr_for_chq_agt_vec { if let Err(e) = item.validate() { return Err(e); } } }
		Ok(())
	}
}


// ChequePresentmentNotificationV02 ...
#[derive(Debug, Default, PartialEq, Clone, Serialize, Deserialize)]
...
impl ChequePresentmentNotificationV02 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if let Err(e) = self.grp_hdr.validate() { return Err(e); }
		for item in &self.chq { if let Err(e) = item.validate() { return Err(e); } }
...
pub struct ExternalChequeAgentInstruction1Code {
	#[serde(rename = "$value")]
	pub external_cheque_agent_instruction1_code: String,
}

impl ExternalChequeAgentInstruction1Code {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if self.external_cheque_agent_instruction1_code.chars().count() < 1 {
			return Err(ValidationError::new(1001, "external_cheque_agent_instruction1_code is shorter than the minimum length of 1".to_string()));
		}
		if self.external_cheque_agent_instruction1_code.chars().count() > 4 {
			return Err(ValidationError::new(1002, "external_cheque_agent_instruction1_code exceeds the maximum length of 4".to_string()));
		}
		Ok(())
	}
}
...
impl GroupHeader103 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Err(e) = self.nb_of_chqs.validate() { return Err(e); }
		Ok(())
	}
}
