...
pub mod iso20022 {
	...
	impl Cheque17 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Some(ref val) = self.pyer { if let Err(e) = val.validate() { return Err(e); } }
			if let Some(ref val) = self.pyer_acct { if let Err(e) = val.validate() { return Err(e); } }
			if let Some(ref val) = self.drwr_agt { if let Err(e) = val.validate() { return Err(e); } }
			if let Some(ref val) = self.drwr_agt_acct { if let Err(e) = val.validate() { return Err(e); } }
			if let Err(e) = self.pyee.validate() { return Err(e); }
			if let Some(ref val) = self.pyee_acct { if let Err(e) = val.validate() { return Err(e); } }
			if let Some(ref vec) = self.instr_for_chq_agt { for item in vec { if let Err(e) = item.validate() { return Err(e); } } }
			Ok(())
		}
	}
