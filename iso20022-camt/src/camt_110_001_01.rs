...
impl AdditionalRequestData1Choice {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if let Some(ref val) = self.reqd_dbt_authstn { if let Err(e) = val.validate() { return Err(e); } }
		if let Some(ref val) = self.reqd_compstn { if let Err(e) = val.validate() { return Err(e); } }
		if let Some(ref val) = self.reqd_valtn { if let Err(e) = val.validate() { return Err(e); } }
...
impl InvestigationReason2 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if let Err(e) = self.rsn.validate() { return Err(e); }
		if let Some(ref val) = self.rsn_sub_tp { if let Err(e) = val.validate() { return Err(e); } }
		if let Some(ref val) = self.addtl_req_data { if let Err(e) = val.validate() { return Err(e); } }
...
impl InvestigationRequest2 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Err(e) = self.undrlyg.validate() { return Err(e); }
...
pub struct InvestigationRequestV01 {
	...
	#[cfg_attr( feature = "derive_serde", serde(rename = "InvstgtnData") )]
...
impl InvestigationRequestV01 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if let Err(e) = self.invstgtn_req.validate() { return Err(e); }
		for item in &self.invstgtn_data { if let Err(e) = item.validate() { return Err(e); } }
