...
impl AdditionalRequestData1Choice {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if let Some(ref reqd_dbt_authstn_value) = self.reqd_dbt_authstn { if let Err(e) = reqd_dbt_authstn_value.validate() { return Err(e); } }
		if let Some(ref reqd_compstn_value) = self.reqd_compstn { if let Err(e) = reqd_compstn_value.validate() { return Err(e); } }
		if let Some(ref reqd_valtn_value) = self.reqd_valtn { if let Err(e) = reqd_valtn_value.validate() { return Err(e); } }
		if let Some(ref req_nrrtv_value) = self.req_nrrtv { if let Err(e) = req_nrrtv_value.validate() { return Err(e); } }
		Ok(())
	}
}
...
impl DebitAuthorisation3 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Some(ref addtl_cxl_rsn_inf_vec) = self.addtl_cxl_rsn_inf { for item in addtl_cxl_rsn_inf_vec { if let Err(e) = item.validate() { return Err(e); } } }
		Ok(())
	}
}
...
impl InvestigationReason2 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if let Err(e) = self.rsn.validate() { return Err(e); }
		if let Some(ref rsn_sub_tp_value) = self.rsn_sub_tp { if let Err(e) = rsn_sub_tp_value.validate() { return Err(e); } }
		if let Some(ref addtl_req_data_value) = self.addtl_req_data { if let Err(e) = addtl_req_data_value.validate() { return Err(e); } }
