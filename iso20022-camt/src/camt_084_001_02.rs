...
pub struct IntraBalanceMovementPostingReportV02 {
	...
	#[serde(rename = "SubBal", skip_serializing_if = "Option::is_none")]
...
impl IntraBalanceMovementPostingReportV02 {
	pub fn validate(&self) -> bool {
		...
		if let Some(ref sub_bal_vec) = self.sub_bal { for item in sub_bal_vec { if !item.validate() { return false; } } }
		return true
	}
}
...
impl IntraBalancePosting5 {
	pub fn validate(&self) -> bool {
		...
		for item in &self.mvmnt { if !item.validate() { return false; } }
		return true
	}
}
...
impl IntraBalancePosting6 {
	pub fn validate(&self) -> bool {
		...
		if !self.sttld_amt.validate() { return false }
		if let Some(ref prevsly_sttld_amt_value) = self.prevsly_sttld_amt { if !prevsly_sttld_amt_value.validate() { return false; } }
		if let Some(ref rmng_sttlm_amt_value) = self.rmng_sttlm_amt { if !rmng_sttlm_amt_value.validate() { return false; } }
		if !self.fctv_sttlm_dt.validate() { return false }
