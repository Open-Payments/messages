...
pub struct GetMemberV04 {
	...
	#[cfg_attr( feature = "derive_serde", serde(rename = "MmbQryDef", skip_serializing_if = "Option::is_none") )]
...
impl GetMemberV04 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Some(ref val) = self.mmb_qry_def { if let Err(e) = val.validate() { return Err(e); } }
...
impl MemberQueryDefinition4 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Some(ref val) = self.mmb_crit { if let Err(e) = val.validate() { return Err(e); } }
		Ok(())
	}
}
