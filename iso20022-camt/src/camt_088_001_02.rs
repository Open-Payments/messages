...
impl NetObligation2 {
	pub fn validate(&self) -> bool {
		...
		if !self.ptcpt_netg_id.validate() { return false }
		if !self.oblgtn_drctn.validate() { return false }
		if !self.ctr_pty_netg_id.validate() { return false }
		if let Some(ref net_svc_ctr_pty_id_value) = self.net_svc_ctr_pty_id { if !net_svc_ctr_pty_id_value.validate() { return false; } }
		if let Some(ref ctr_pty_sttlm_instrs_value) = self.ctr_pty_sttlm_instrs { if !ctr_pty_sttlm_instrs_value.validate() { return false; } }
		if let Some(ref txs_nb_value) = self.txs_nb { if !txs_nb_value.validate() { return false; } }
		return true
	}
}
...
impl NetReportData2 {
	pub fn validate(&self) -> bool {
		...
		if let Some(ref net_rpt_svcr_value) = self.net_rpt_svcr { if !net_rpt_svcr_value.validate() { return false; } }
		if let Some(ref net_svc_tp_value) = self.net_svc_tp { if !net_svc_tp_value.validate() { return false; } }
...
impl NetReportV02 {
	pub fn validate(&self) -> bool {
		if !self.net_rpt_data.validate() { return false }
		if !self.net_svc_ptcpt_id.validate() { return false }
		if let Some(ref net_svc_ctr_pty_id_value) = self.net_svc_ctr_pty_id { if !net_svc_ctr_pty_id_value.validate() { return false; } }
		for item in &self.net_oblgtn { if !item.validate() { return false; } }
...
impl NettingIdentification2Choice {
	pub fn validate(&self) -> bool {
		if let Some(ref trad_pty_value) = self.trad_pty { if !trad_pty_value.validate() { return false; } }
		if let Some(ref netg_grp_id_value) = self.netg_grp_id { if !netg_grp_id_value.validate() { return false; } }
		return true
	}
}
...
impl SettlementParties120 {
	pub fn validate(&self) -> bool {
		if let Some(ref dlvry_agt_value) = self.dlvry_agt { if !dlvry_agt_value.validate() { return false; } }
		if let Some(ref intrmy_value) = self.intrmy { if !intrmy_value.validate() { return false; } }
		if !self.rcvg_agt.validate() { return false }
		if let Some(ref bnfcry_instn_value) = self.bnfcry_instn { if !bnfcry_instn_value.validate() { return false; } }
		return true
	}
}
