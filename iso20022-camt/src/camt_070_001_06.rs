...
impl StandingOrderOrError10Choice {
	pub fn validate(&self) -> bool {
		if let Some(ref stg_ordr_value) = self.stg_ordr { if !stg_ordr_value.validate() { return false; } }
...
impl StandingOrderReport3 {
	pub fn validate(&self) -> bool {
		if !self.stg_ordr_id.validate() { return false }
		if !self.stg_ordr_or_err.validate() { return false }
		return true
	}
}
