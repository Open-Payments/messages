...
pub mod iso20022 {
	...
	impl IntraBalanceModification8 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Some(ref val) = self.req_dtls { if let Err(e) = val.validate() { return Err(e); } }
	...
	impl IntraBalanceOrOperationalError12Choice {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if let Some(ref vec) = self.mods { for item in vec { if let Err(e) = item.validate() { return Err(e); } } }
