...
pub mod iso20022 {
	...
	// IntraBalanceMovementCancellationQueryV02 ...
	#[cfg_attr(feature = "derive_debug", derive(Debug))]
	#[cfg_attr(feature = "derive_clone", derive(Clone))]
	#[cfg_attr(feature = "derive_partial_eq", derive(PartialEq))]
	#[cfg_attr(feature = "derive_default", derive(Default))]
	...
	impl IntraBalanceMovementCancellationQueryV02 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if let Some(ref id_value) = self.id { if let Err(e) = id_value.validate() { return Err(e); } }
	...
	pub struct IntraBalanceQueryCriteria10 {
		...
		pub cxl_req_id: Option<Vec<Max35Text>>,
	...
	impl IntraBalanceQueryCriteria10 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if let Some(ref cxl_req_id_vec) = self.cxl_req_id { for item in cxl_req_id_vec { if let Err(e) = item.validate() { return Err(e); } } }
