...
impl Cheque20 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Some(ref val) = self.pyee { if let Err(e) = val.validate() { return Err(e); } }
		if let Some(ref val) = self.pyee_acct { if let Err(e) = val.validate() { return Err(e); } }
		if let Some(ref val) = self.chq_cxl_or_stop_rsn { if let Err(e) = val.validate() { return Err(e); } }
		Ok(())
	}
}
