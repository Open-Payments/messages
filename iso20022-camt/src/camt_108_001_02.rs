...
impl Cheque20 {
	pub fn validate(&self) -> bool {
		...
		if let Some(ref pyee_value) = self.pyee { if !pyee_value.validate() { return false; } }
		if let Some(ref pyee_acct_value) = self.pyee_acct { if !pyee_acct_value.validate() { return false; } }
		if let Some(ref chq_cxl_or_stop_rsn_value) = self.chq_cxl_or_stop_rsn { if !chq_cxl_or_stop_rsn_value.validate() { return false; } }
		return true
	}
}
