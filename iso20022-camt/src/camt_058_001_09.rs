...
pub mod iso20022 {
	...
	impl ExternalNotificationCancellationReason1Code {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			return Err(ValidationError::new(1001, "external_notification_cancellation_reason1_code is shorter than the minimum length of 1".to_string()));
			}
			if self.external_notification_cancellation_reason1_code.chars().count() > 4 {
				return Err(ValidationError::new(1002, "external_notification_cancellation_reason1_code exceeds the maximum length of 4".to_string()));
			}
			Ok(())
		}
	}
	...
	pub struct NotificationToReceiveCancellationAdviceV09 {
		...
		#[cfg_attr( feature = "derive_serde", serde(rename = "OrgnlNtfctn") )]
	...
	impl NotificationToReceiveCancellationAdviceV09 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Err(e) = self.orgnl_ntfctn.validate() { return Err(e); }
	...
	impl OriginalNotificationReference14 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Some(ref dbtr_agt_value) = self.dbtr_agt { if let Err(e) = dbtr_agt_value.validate() { return Err(e); } }
			if let Some(ref intrmy_agt_value) = self.intrmy_agt { if let Err(e) = intrmy_agt_value.validate() { return Err(e); } }
			for item in &self.orgnl_itm { if let Err(e) = item.validate() { return Err(e); } }
			Ok(())
		}
	}
