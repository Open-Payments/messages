...
pub mod iso20022 {
	...
	impl ClaimNonReceiptV10 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Some(ref val) = self.case { if let Err(e) = val.validate() { return Err(e); } }
			if let Err(e) = self.undrlyg.validate() { return Err(e); }
			if let Some(ref val) = self.cover_dtls { if let Err(e) = val.validate() { return Err(e); } }
			if let Some(ref val) = self.instr_for_assgne { if let Err(e) = val.validate() { return Err(e); } }
	...
	impl MissingCover6 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if let Some(ref val) = self.cover_crrctn { if let Err(e) = val.validate() { return Err(e); } }
			Ok(())
		}
	}
