...
pub mod iso20022 {
	...
	impl Limit7 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Some(ref val) = self.usd_amt_cdt_dbt_ind { if let Err(e) = val.validate() { return Err(e); } }
			if let Some(ref val) = self.rmng_amt { if let Err(e) = val.validate() { return Err(e); } }
			Ok(())
		}
	}
	...
	impl LimitOrError4Choice {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if let Some(ref val) = self.lmt { if let Err(e) = val.validate() { return Err(e); } }
	...
	impl Limits8 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if let Some(ref vec) = self.cur_lmt { for item in vec { if let Err(e) = item.validate() { return Err(e); } } }
			if let Some(ref vec) = self.dflt_lmt { for item in vec { if let Err(e) = item.validate() { return Err(e); } } }
			Ok(())
		}
	}
