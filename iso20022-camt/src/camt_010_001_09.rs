...
impl Limit7 {
	pub fn validate(&self) -> bool {
		...
		if let Some(ref usd_amt_cdt_dbt_ind_value) = self.usd_amt_cdt_dbt_ind { if !usd_amt_cdt_dbt_ind_value.validate() { return false; } }
		if let Some(ref rmng_amt_value) = self.rmng_amt { if !rmng_amt_value.validate() { return false; } }
		return true
	}
}
...
impl LimitOrError4Choice {
	pub fn validate(&self) -> bool {
		if let Some(ref lmt_value) = self.lmt { if !lmt_value.validate() { return false; } }
...
impl LimitReport8 {
	pub fn validate(&self) -> bool {
		...
		if !self.lmt_or_err.validate() { return false }
		return true
	}
}
...
#[derive(Debug, Default, PartialEq, Clone, Serialize, Deserialize)]
pub struct Limits8 {
	#[serde(rename = "CurLmt", skip_serializing_if = "Option::is_none")]
	pub cur_lmt: Option<Vec<LimitReport8>>,
	#[serde(rename = "DfltLmt", skip_serializing_if = "Option::is_none")]
	pub dflt_lmt: Option<Vec<LimitReport8>>,
}

impl Limits8 {
	pub fn validate(&self) -> bool {
		if let Some(ref cur_lmt_vec) = self.cur_lmt { for item in cur_lmt_vec { if !item.validate() { return false; } } }
		if let Some(ref dflt_lmt_vec) = self.dflt_lmt { for item in dflt_lmt_vec { if !item.validate() { return false; } } }
		return true
	}
}
