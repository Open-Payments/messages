...
#[derive(Debug, Default, PartialEq, Clone, Serialize, Deserialize)]
#[serde(transparent)]
pub struct ExternalCreditorEnrolmentCancellationReason1Code {
	#[serde(rename = "$value")]
	pub external_creditor_enrolment_cancellation_reason1_code: String,
}

impl ExternalCreditorEnrolmentCancellationReason1Code {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if self.external_creditor_enrolment_cancellation_reason1_code.chars().count() < 1 {
			return Err(ValidationError::new(1001, "external_creditor_enrolment_cancellation_reason1_code is shorter than the minimum length of 1".to_string()));
		}
		if self.external_creditor_enrolment_cancellation_reason1_code.chars().count() > 4 {
			return Err(ValidationError::new(1002, "external_creditor_enrolment_cancellation_reason1_code exceeds the maximum length of 4".to_string()));
		}
		Ok(())
	}
}
