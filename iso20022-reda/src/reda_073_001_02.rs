...
impl ActivationStatus3 {
	pub fn validate(&self) -> bool {
		...
		if let Some(ref orgnl_actvtn_ref_value) = self.orgnl_actvtn_ref { if !orgnl_actvtn_ref_value.validate() { return false; } }
		if let Some(ref fctv_actvtn_dt_value) = self.fctv_actvtn_dt { if !fctv_actvtn_dt_value.validate() { return false; } }
...
impl RequestToPayDebtorActivationStatusReportV02 {
	pub fn validate(&self) -> bool {
		...
		for item in &self.orgnl_actvtn_and_sts { if !item.validate() { return false; } }
		if let Some(ref splmtry_data_vec) = self.splmtry_data { for item in splmtry_data_vec { if !item.validate() { return false; } } }
		return true
	}
}
