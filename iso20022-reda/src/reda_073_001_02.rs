...
pub struct ActivationStatus3 {
	...
	#[serde(rename = "OrgnlActvtnRef", skip_serializing_if = "Option::is_none")]
	pub orgnl_actvtn_ref: Option<OriginalActivation3Choice>,
	#[serde(rename = "FctvActvtnDt", skip_serializing_if = "Option::is_none")]
...
impl ActivationStatus3 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Some(ref orgnl_actvtn_ref_value) = self.orgnl_actvtn_ref { if let Err(e) = orgnl_actvtn_ref_value.validate() { return Err(e); } }
		if let Some(ref fctv_actvtn_dt_value) = self.fctv_actvtn_dt { if let Err(e) = fctv_actvtn_dt_value.validate() { return Err(e); } }
...
impl ExternalDebtorActivationStatusReason1Code {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if self.external_debtor_activation_status_reason1_code.chars().count() < 1 {
			return Err(ValidationError::new(1001, "external_debtor_activation_status_reason1_code is shorter than the minimum length of 1".to_string()));
		}
		if self.external_debtor_activation_status_reason1_code.chars().count() > 4 {
			return Err(ValidationError::new(1002, "external_debtor_activation_status_reason1_code exceeds the maximum length of 4".to_string()));
		}
		Ok(())
	}
}
...
impl RequestToPayDebtorActivationStatusReportV02 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if let Err(e) = self.hdr.validate() { return Err(e); }
		for item in &self.orgnl_actvtn_and_sts { if let Err(e) = item.validate() { return Err(e); } }
		if let Some(ref splmtry_data_vec) = self.splmtry_data { for item in splmtry_data_vec { if let Err(e) = item.validate() { return Err(e); } } }
		Ok(())
	}
}
