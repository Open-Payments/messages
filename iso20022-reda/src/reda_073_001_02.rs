...
impl ActivationStatus3 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Some(ref val) = self.orgnl_actvtn_ref { if let Err(e) = val.validate() { return Err(e); } }
		if let Some(ref val) = self.fctv_actvtn_dt { if let Err(e) = val.validate() { return Err(e); } }
...
impl DebtorActivation5 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Some(ref vec) = self.cstmr_id { for item in vec { if let Err(e) = item.validate() { return Err(e); } } }
		if let Some(ref vec) = self.ctrct_frmt_tp { for item in vec { if let Err(e) = item.validate() { return Err(e); } } }
		if let Some(ref vec) = self.ctrct_ref { for item in vec { if let Err(e) = item.validate() { return Err(e); } } }
...
impl OriginalActivation3Choice {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if let Some(ref val) = self.orgnl_dbtr_id { if let Err(e) = val.validate() { return Err(e); } }
		if let Some(ref val) = self.orgnl_actvtn_data { if let Err(e) = val.validate() { return Err(e); } }
		Ok(())
	}
}
