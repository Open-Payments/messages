...
impl PartyStatement3 {
	pub fn validate(&self) -> bool {
		if let Some(ref chng_vec) = self.chng { for item in chng_vec { if !item.validate() { return false; } } }
		return true
	}
}
