...
pub mod iso20022 {
	...
	pub struct PartyActivityAdviceV02 {
		...
		#[cfg_attr( feature = "derive_serde", serde(rename = "PtyActvty") )]
	...
	impl PartyActivityAdviceV02 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Err(e) = self.pty_actvty.validate() { return Err(e); }
	...
	impl PartyStatement3 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if let Some(ref chng_vec) = self.chng { for item in chng_vec { if let Err(e) = item.validate() { return Err(e); } } }
			Ok(())
		}
	}
