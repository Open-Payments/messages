...
impl CommonFinancialInstrumentAttributes12 {
	pub fn validate(&self) -> bool {
		...
		if let Some(ref iso_scty_lng_nm_value) = self.iso_scty_lng_nm { if !iso_scty_lng_nm_value.validate() { return false; } }
		if let Some(ref iso_scty_shrt_nm_value) = self.iso_scty_shrt_nm { if !iso_scty_shrt_nm_value.validate() { return false; } }
		if let Some(ref nm_vld_fr_value) = self.nm_vld_fr { if !nm_vld_fr_value.validate() { return false; } }
...
impl ExternalSecuritiesUpdateReason1Code {
	pub fn validate(&self) -> bool {
		if self.external_securities_update_reason1_code.chars().count() < 1 {
			return false
		}
		if self.external_securities_update_reason1_code.chars().count() > 4 {
			return false
		}
		return true
	}
...
impl SecurityAttributes12 {
	pub fn validate(&self) -> bool {
		if let Some(ref fin_instrm_tp_vec) = self.fin_instrm_tp { for item in fin_instrm_tp_vec { if !item.validate() { return false; } } }
...
impl SecurityMaintenanceRequestV01 {
	pub fn validate(&self) -> bool {
		...
		if let Some(ref upd_rsn_value) = self.upd_rsn { if !upd_rsn_value.validate() { return false; } }
...
impl UpdateType35Choice {
	pub fn validate(&self) -> bool {
		if let Some(ref add_value) = self.add { if !add_value.validate() { return false; } }
		if let Some(ref del_value) = self.del { if !del_value.validate() { return false; } }
		if let Some(ref modfy_value) = self.modfy { if !modfy_value.validate() { return false; } }
		return true
	}
}
...
impl UpdateType36Choice {
	pub fn validate(&self) -> bool {
		if let Some(ref upd_tp_vec) = self.upd_tp { for item in upd_tp_vec { if !item.validate() { return false; } } }
		if let Some(ref rplc_value) = self.rplc { if !rplc_value.validate() { return false; } }
		return true
	}
}
