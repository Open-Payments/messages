...
pub mod iso20022 {
	...
	impl ExternalSecuritiesUpdateReason1Code {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			return Err(ValidationError::new(1001, "external_securities_update_reason1_code is shorter than the minimum length of 1".to_string()));
			}
			if self.external_securities_update_reason1_code.chars().count() > 4 {
				return Err(ValidationError::new(1002, "external_securities_update_reason1_code exceeds the maximum length of 4".to_string()));
			}
			Ok(())
		}
	}
	...
	impl SecurityMaintenanceRequestV01 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Some(ref upd_rsn_value) = self.upd_rsn { if let Err(e) = upd_rsn_value.validate() { return Err(e); } }
	...
	impl UpdateType35Choice {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if let Some(ref add_value) = self.add { if let Err(e) = add_value.validate() { return Err(e); } }
			if let Some(ref del_value) = self.del { if let Err(e) = del_value.validate() { return Err(e); } }
			if let Some(ref modfy_value) = self.modfy { if let Err(e) = modfy_value.validate() { return Err(e); } }
			Ok(())
		}
	}
	...
	impl UpdateType36Choice {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if let Some(ref upd_tp_vec) = self.upd_tp { for item in upd_tp_vec { if let Err(e) = item.validate() { return Err(e); } } }
			if let Some(ref rplc_value) = self.rplc { if let Err(e) = rplc_value.validate() { return Err(e); } }
			Ok(())
		}
	}
