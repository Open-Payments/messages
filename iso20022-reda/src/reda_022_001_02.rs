...
impl PartyModificationRequestV02 {
	pub fn validate(&self) -> bool {
		...
		if !self.sys_pty_id.validate() { return false }
...
impl SystemPartyModification3Choice {
	pub fn validate(&self) -> bool {
		if let Some(ref sys_pty_dt_value) = self.sys_pty_dt { if !sys_pty_dt_value.validate() { return false; } }
		...
		if let Some(ref pty_adr_value) = self.pty_adr { if !pty_adr_value.validate() { return false; } }
