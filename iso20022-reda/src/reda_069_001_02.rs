...
impl EnrolmentStatus3 {
	pub fn validate(&self) -> bool {
		...
		if let Some(ref orgnl_enrlmnt_ref_value) = self.orgnl_enrlmnt_ref { if !orgnl_enrlmnt_ref_value.validate() { return false; } }
		if let Some(ref fctv_enrlmnt_dt_value) = self.fctv_enrlmnt_dt { if !fctv_enrlmnt_dt_value.validate() { return false; } }
...
impl RequestToPayCreditorEnrolmentStatusReportV02 {
	pub fn validate(&self) -> bool {
		...
		for item in &self.orgnl_enrlmnt_and_sts { if !item.validate() { return false; } }
		if let Some(ref splmtry_data_vec) = self.splmtry_data { for item in splmtry_data_vec { if !item.validate() { return false; } } }
		return true
	}
}
