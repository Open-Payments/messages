...
pub struct EnrolmentStatus3 {
	...
	#[serde(rename = "OrgnlEnrlmntRef", skip_serializing_if = "Option::is_none")]
	pub orgnl_enrlmnt_ref: Option<OriginalEnrolment3Choice>,
	#[serde(rename = "FctvEnrlmntDt", skip_serializing_if = "Option::is_none")]
...
impl EnrolmentStatus3 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Some(ref orgnl_enrlmnt_ref_value) = self.orgnl_enrlmnt_ref { if let Err(e) = orgnl_enrlmnt_ref_value.validate() { return Err(e); } }
		if let Some(ref fctv_enrlmnt_dt_value) = self.fctv_enrlmnt_dt { if let Err(e) = fctv_enrlmnt_dt_value.validate() { return Err(e); } }
...
#[derive(Debug, Default, PartialEq, Clone, Serialize, Deserialize)]
#[serde(transparent)]
pub struct ExternalCreditorEnrolmentStatusReason1Code {
	#[serde(rename = "$value")]
	pub external_creditor_enrolment_status_reason1_code: String,
}

impl ExternalCreditorEnrolmentStatusReason1Code {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if self.external_creditor_enrolment_status_reason1_code.chars().count() < 1 {
			return Err(ValidationError::new(1001, "external_creditor_enrolment_status_reason1_code is shorter than the minimum length of 1".to_string()));
		}
		if self.external_creditor_enrolment_status_reason1_code.chars().count() > 4 {
			return Err(ValidationError::new(1002, "external_creditor_enrolment_status_reason1_code exceeds the maximum length of 4".to_string()));
		}
		Ok(())
	}
}
...
// RequestToPayCreditorEnrolmentStatusReportV02 ...
#[derive(Debug, Default, PartialEq, Clone, Serialize, Deserialize)]
pub struct RequestToPayCreditorEnrolmentStatusReportV02 {
	#[serde(rename = "Hdr")]
	pub hdr: EnrolmentHeader3,
	#[serde(rename = "OrgnlEnrlmntAndSts")]
	pub orgnl_enrlmnt_and_sts: Vec<EnrolmentStatus3>,
	#[serde(rename = "SplmtryData", skip_serializing_if = "Option::is_none")]
	pub splmtry_data: Option<Vec<SupplementaryData1>>,
}

impl RequestToPayCreditorEnrolmentStatusReportV02 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if let Err(e) = self.hdr.validate() { return Err(e); }
		for item in &self.orgnl_enrlmnt_and_sts { if let Err(e) = item.validate() { return Err(e); } }
		if let Some(ref splmtry_data_vec) = self.splmtry_data { for item in splmtry_data_vec { if let Err(e) = item.validate() { return Err(e); } } }
		Ok(())
	}
}
