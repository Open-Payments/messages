...
pub mod iso20022 {
	...
	impl DebtorActivation6 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Some(ref val) = self.dbtr_sol_prvdr { if let Err(e) = val.validate() { return Err(e); } }
			if let Some(ref vec) = self.cstmr_id { for item in vec { if let Err(e) = item.validate() { return Err(e); } } }
			if let Some(ref vec) = self.ctrct_frmt_tp { for item in vec { if let Err(e) = item.validate() { return Err(e); } } }
			if let Some(ref vec) = self.ctrct_ref { for item in vec { if let Err(e) = item.validate() { return Err(e); } } }
	...
	impl DebtorActivationAmendment6 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if let Some(ref val) = self.dbtr_actvtn { if let Err(e) = val.validate() { return Err(e); } }
			if let Some(ref val) = self.elctrnc_invc_data { if let Err(e) = val.validate() { return Err(e); } }
			Ok(())
		}
	}
