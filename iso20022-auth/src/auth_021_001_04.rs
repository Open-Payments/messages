...
pub mod iso20022 {
	...
	pub struct ContractRegistrationAmendmentRequestV04 {
		...
		#[cfg_attr( feature = "derive_serde", serde(rename = "CtrctRegnAmdmnt") )]
	...
	impl ContractRegistrationAmendmentRequestV04 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			for item in &self.ctrct_regn_amdmnt { if let Err(e) = item.validate() { return Err(e); } }
	...
	impl RegisteredContract16 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if let Err(e) = self.ctrct_regn_amdmnt_id.validate() { return Err(e); }
			if let Err(e) = self.rptg_pty.validate() { return Err(e); }
			if let Err(e) = self.regn_agt.validate() { return Err(e); }
			for item in &self.regd_ctrct_amdmnt { if let Err(e) = item.validate() { return Err(e); } }
			if let Some(ref splmtry_data_vec) = self.splmtry_data { for item in splmtry_data_vec { if let Err(e) = item.validate() { return Err(e); } } }
			Ok(())
		}
	}
	...
	impl RegisteredContract17 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if let Err(e) = self.regd_ctrct_amdmnt_id.validate() { return Err(e); }
			if let Err(e) = self.orgnl_regd_ctrct_id.validate() { return Err(e); }
