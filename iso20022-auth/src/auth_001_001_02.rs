...
impl InformationRequestOpeningV02 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Err(e) = self.lgl_mndt_bsis.validate() { return Err(e); }
		if let Some(ref val) = self.due_dt { if let Err(e) = val.validate() { return Err(e); } }
		if let Err(e) = self.invstgtn_prd.validate() { return Err(e); }
