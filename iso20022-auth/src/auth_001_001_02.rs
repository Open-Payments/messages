...
impl InformationRequestOpeningV02 {
	pub fn validate(&self) -> bool {
		...
		if !self.lgl_mndt_bsis.validate() { return false }
		if let Some(ref due_dt_value) = self.due_dt { if !due_dt_value.validate() { return false; } }
		if !self.invstgtn_prd.validate() { return false }
...
impl LegalMandate1 {
	pub fn validate(&self) -> bool {
		if !self.prgrph.validate() { return false }
		if let Some(ref dsclmr_value) = self.dsclmr { if !dsclmr_value.validate() { return false; } }
		return true
	}
}
