...
pub mod iso20022 {
	...
	impl ContractCessionData2 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Some(ref doc_nb_value) = self.doc_nb { if let Err(e) = doc_nb_value.validate() { return Err(e); } }
			Ok(())
		}
	}
	...
	pub struct ContractRegistrationClosureRequestV04 {
		...
		#[cfg_attr( feature = "derive_serde", serde(rename = "RegdCtrctClsr") )]
	...
	impl ContractRegistrationClosureRequestV04 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			for item in &self.regd_ctrct_clsr { if let Err(e) = item.validate() { return Err(e); } }
	...
	impl RegisteredContract19 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if let Err(e) = self.regd_ctrct_clsr_id.validate() { return Err(e); }
			if let Err(e) = self.rptg_pty.validate() { return Err(e); }
			if let Err(e) = self.regn_agt.validate() { return Err(e); }
			if let Err(e) = self.orgnl_regd_ctrct.validate() { return Err(e); }
			if let Err(e) = self.prty.validate() { return Err(e); }
			if let Err(e) = self.clsr_rsn.validate() { return Err(e); }
			if let Some(ref cssn_value) = self.cssn { if let Err(e) = cssn_value.validate() { return Err(e); } }
