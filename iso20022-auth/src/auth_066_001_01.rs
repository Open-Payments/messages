...
impl CCPBackTestingResultReportV01 {
	pub fn validate(&self) -> bool {
		for item in &self.mnthly_rslt { if !item.validate() { return false; } }
...
impl MonthlyResult1 {
	pub fn validate(&self) -> bool {
		if !self.lrgst_xcptn.validate() { return false }
		if !self.avrg_xcptn.validate() { return false }
		if let Some(ref lrgst_xcptn_id_value) = self.lrgst_xcptn_id { if !lrgst_xcptn_id_value.validate() { return false; } }
		return true
	}
}
