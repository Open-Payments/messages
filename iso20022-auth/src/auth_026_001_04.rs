...
pub mod iso20022 {
	...
	impl OriginalMessage6 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if let Some(ref val) = self.orgnl_sndr { if let Err(e) = val.validate() { return Err(e); } }
	...
	impl SupportingDocumentRequestOrLetter4 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Some(ref val) = self.sndr { if let Err(e) = val.validate() { return Err(e); } }
			if let Some(ref val) = self.rcvr { if let Err(e) = val.validate() { return Err(e); } }
			if let Some(ref vec) = self.orgnl_refs { for item in vec { if let Err(e) = item.validate() { return Err(e); } } }
