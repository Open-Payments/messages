...
pub struct ExternalLetterType1Code {
	#[serde(rename = "$value")]
	pub external_letter_type1_code: String,
}

impl ExternalLetterType1Code {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if self.external_letter_type1_code.chars().count() < 1 {
			return Err(ValidationError::new(1001, "external_letter_type1_code is shorter than the minimum length of 1".to_string()));
		}
		if self.external_letter_type1_code.chars().count() > 4 {
			return Err(ValidationError::new(1002, "external_letter_type1_code exceeds the maximum length of 4".to_string()));
		}
		Ok(())
	}
}
...
impl OriginalMessage6 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Some(ref orgnl_packg_id_value) = self.orgnl_packg_id { if let Err(e) = orgnl_packg_id_value.validate() { return Err(e); } }
...
pub struct SupportingDocumentRequestOrLetter4 {
	...
	#[serde(rename = "OrgnlRefs", skip_serializing_if = "Option::is_none")]
...
impl SupportingDocumentRequestOrLetter4 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if let Err(e) = self.req_or_lttr_id.validate() { return Err(e); }
		if let Some(ref sndr_value) = self.sndr { if let Err(e) = sndr_value.validate() { return Err(e); } }
		if let Some(ref rcvr_value) = self.rcvr { if let Err(e) = rcvr_value.validate() { return Err(e); } }
		if let Some(ref orgnl_refs_vec) = self.orgnl_refs { for item in orgnl_refs_vec { if let Err(e) = item.validate() { return Err(e); } } }
		if let Err(e) = self.sbjt.validate() { return Err(e); }
