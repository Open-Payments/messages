...
pub struct FinancialInstrumentReportingTradingVolumeCapDataReportV01 {
	...
	#[cfg_attr( feature = "derive_serde", serde(rename = "VolCapData") )]
	pub vol_cap_data: Vec<VolumeCapReport1>,
	#[cfg_attr( feature = "derive_serde", serde(rename = "SplmtryData", skip_serializing_if = "Option::is_none") )]
	pub splmtry_data: Option<Vec<SupplementaryData1>>,
}

impl FinancialInstrumentReportingTradingVolumeCapDataReportV01 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if let Err(e) = self.rpt_hdr.validate() { return Err(e); }
		for item in &self.vol_cap_data { if let Err(e) = item.validate() { return Err(e); } }
...
impl VolumeCapReport1 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		for item in &self.instrm_rpt { if let Err(e) = item.validate() { return Err(e); } }
		Ok(())
	}
}
