...
impl FinancialInstrumentReportingReferenceDataDeltaReportV03 {
	pub fn validate(&self) -> bool {
		...
		if let Some(ref fin_instrm_vec) = self.fin_instrm { for item in fin_instrm_vec { if !item.validate() { return false; } } }
...
impl SecuritiesReferenceDeltaStatusReport5Choice {
	pub fn validate(&self) -> bool {
		if let Some(ref modfd_rcrd_value) = self.modfd_rcrd { if !modfd_rcrd_value.validate() { return false; } }
		if let Some(ref new_rcrd_value) = self.new_rcrd { if !new_rcrd_value.validate() { return false; } }
		if let Some(ref termntd_rcrd_value) = self.termntd_rcrd { if !termntd_rcrd_value.validate() { return false; } }
		if let Some(ref canc_rcrd_value) = self.canc_rcrd { if !canc_rcrd_value.validate() { return false; } }
		return true
	}
}
