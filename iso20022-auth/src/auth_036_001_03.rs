...
impl FinancialInstrumentReportingReferenceDataDeltaReportV03 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Some(ref vec) = self.fin_instrm { for item in vec { if let Err(e) = item.validate() { return Err(e); } } }
...
impl SecuritiesReferenceDeltaStatusReport5Choice {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if let Some(ref val) = self.modfd_rcrd { if let Err(e) = val.validate() { return Err(e); } }
		if let Some(ref val) = self.new_rcrd { if let Err(e) = val.validate() { return Err(e); } }
		if let Some(ref val) = self.termntd_rcrd { if let Err(e) = val.validate() { return Err(e); } }
		if let Some(ref val) = self.canc_rcrd { if let Err(e) = val.validate() { return Err(e); } }
		Ok(())
	}
}
