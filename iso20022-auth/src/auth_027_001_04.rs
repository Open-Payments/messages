...
impl CurrencyControlRecordStatus3 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Err(e) = self.sts.validate() { return Err(e); }
		if let Some(ref vec) = self.sts_rsn { for item in vec { if let Err(e) = item.validate() { return Err(e); } } }
		if let Some(ref val) = self.doc_id { if let Err(e) = val.validate() { return Err(e); } }
		Ok(())
	}
}
...
impl CurrencyControlStatusAdviceV04 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Some(ref vec) = self.packg_sts { for item in vec { if let Err(e) = item.validate() { return Err(e); } } }
