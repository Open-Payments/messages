...
pub mod iso20022 {
	...
	impl NumberAndVolume2 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if let Err(e) = self.vol.validate() { return Err(e); }
			Ok(())
		}
	}
	...
	impl TransparencyDataReport13 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Err(e) = self.txs_exctd.validate() { return Err(e); }
			if let Err(e) = self.txs_exctd_exclg_pre_trad_wvr.validate() { return Err(e); }
			if let Err(e) = self.txs_exctd_exclg_pst_trad_lrg_in_scale_wvr.validate() { return Err(e); }
			Ok(())
		}
	}
