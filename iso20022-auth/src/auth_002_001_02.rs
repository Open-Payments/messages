...
pub mod iso20022 {
	...
	impl AuthorityInvestigation2 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Some(ref val) = self.addtl_invstgtd_pties { if let Err(e) = val.validate() { return Err(e); } }
	...
	impl InformationRequestResponseV02 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if self.rspn_id.chars().count() < 1 {
				return Err(ValidationError::new(1001, "rspn_id is shorter than the minimum length of 1".to_string()));
			}
			if self.rspn_id.chars().count() > 35 {
				return Err(ValidationError::new(1002, "rspn_id exceeds the maximum length of 35".to_string()));
			}
	...
	impl InvestigationResult1Choice {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if let Some(ref val) = self.rslt { if let Err(e) = val.validate() { return Err(e); } }
			if let Some(ref val) = self.invstgtn_sts { if let Err(e) = val.validate() { return Err(e); } }
			Ok(())
		}
	}
	...
	impl ReturnIndicator2 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if let Some(ref val) = self.rspn_prd { if let Err(e) = val.validate() { return Err(e); } }
	...
	impl SearchCriteria2Choice {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Some(ref val) = self.cstmr_id { if let Err(e) = val.validate() { return Err(e); } }
			if let Some(ref val) = self.pmt_instrm { if let Err(e) = val.validate() { return Err(e); } }
			if let Some(ref vec) = self.orgnl_tx_nb { for item in vec { if let Err(e) = item.validate() { return Err(e); } } }
			Ok(())
		}
	}
