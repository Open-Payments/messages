...
impl OriginalGroupInformation30 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Some(ref vec) = self.nb_of_txs_per_sts { for item in vec { if let Err(e) = item.validate() { return Err(e); } } }
		Ok(())
	}
}
...
impl PaymentConditionStatus1 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if let Some(ref val) = self.accptd_amt { if let Err(e) = val.validate() { return Err(e); } }
		Ok(())
	}
}
...
impl PaymentTransaction104 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Some(ref vec) = self.sts_rsn_inf { for item in vec { if let Err(e) = item.validate() { return Err(e); } } }
		if let Some(ref val) = self.pmt_cond_sts { if let Err(e) = val.validate() { return Err(e); } }
