...
impl CreditorPaymentActivationRequestStatusReportV07 {
	pub fn validate(&self) -> bool {
		if !self.grp_hdr.validate() { return false }
		if !self.orgnl_grp_inf_and_sts.validate() { return false }
...
impl OriginalGroupInformation30 {
	pub fn validate(&self) -> bool {
		...
		if let Some(ref grp_sts_value) = self.grp_sts { if !grp_sts_value.validate() { return false; } }
		if let Some(ref sts_rsn_inf_vec) = self.sts_rsn_inf { for item in sts_rsn_inf_vec { if !item.validate() { return false; } } }
		if let Some(ref nb_of_txs_per_sts_vec) = self.nb_of_txs_per_sts { for item in nb_of_txs_per_sts_vec { if !item.validate() { return false; } } }
		return true
	}
}
...
impl OriginalPaymentInstruction31 {
	pub fn validate(&self) -> bool {
		...
		if let Some(ref orgnl_nb_of_txs_value) = self.orgnl_nb_of_txs { if !orgnl_nb_of_txs_value.validate() { return false; } }
		if let Some(ref pmt_inf_sts_value) = self.pmt_inf_sts { if !pmt_inf_sts_value.validate() { return false; } }
		if let Some(ref sts_rsn_inf_vec) = self.sts_rsn_inf { for item in sts_rsn_inf_vec { if !item.validate() { return false; } } }
		if let Some(ref nb_of_txs_per_sts_vec) = self.nb_of_txs_per_sts { for item in nb_of_txs_per_sts_vec { if !item.validate() { return false; } } }
...
impl PaymentConditionStatus1 {
	pub fn validate(&self) -> bool {
		if let Some(ref accptd_amt_value) = self.accptd_amt { if !accptd_amt_value.validate() { return false; } }
		return true
	}
}
...
impl PaymentTransaction104 {
	pub fn validate(&self) -> bool {
		if let Some(ref sts_id_value) = self.sts_id { if !sts_id_value.validate() { return false; } }
		...
		if let Some(ref sts_rsn_inf_vec) = self.sts_rsn_inf { for item in sts_rsn_inf_vec { if !item.validate() { return false; } } }
		if let Some(ref pmt_cond_sts_value) = self.pmt_cond_sts { if !pmt_cond_sts_value.validate() { return false; } }
