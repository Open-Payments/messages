...
impl ResendRequestV01 {
	pub fn validate(&self) -> bool {
		...
		for item in &self.rsnd_sch_crit { if !item.validate() { return false; } }
...
impl ResendSearchCriteria2 {
	pub fn validate(&self) -> bool {
		...
		if let Some(ref seq_rg_value) = self.seq_rg { if !seq_rg_value.validate() { return false; } }
		if let Some(ref orgnl_msg_nm_id_value) = self.orgnl_msg_nm_id { if !orgnl_msg_nm_id_value.validate() { return false; } }
		if let Some(ref file_ref_value) = self.file_ref { if !file_ref_value.validate() { return false; } }
