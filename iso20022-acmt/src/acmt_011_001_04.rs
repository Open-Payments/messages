...
pub mod iso20022 {
	...
	impl References6 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if let Err(e) = self.rjctd_req_tp.validate() { return Err(e); }
			for item in &self.rjctn_rsn { if let Err(e) = item.validate() { return Err(e); } }
			if let Err(e) = self.rjctd_req_id.validate() { return Err(e); }
