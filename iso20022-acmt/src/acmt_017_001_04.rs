...
impl AccountMandateMaintenanceRequestV04 {
	pub fn validate(&self) -> bool {
		...
		for item in &self.acct_id { if !item.validate() { return false; } }
		if !self.acct_svcr_id.validate() { return false }
		if !self.org_id.validate() { return false }
		for item in &self.mndt { if !item.validate() { return false; } }
...
impl Organisation44 {
	pub fn validate(&self) -> bool {
		if let Some(ref full_lgl_nm_value) = self.full_lgl_nm { if !full_lgl_nm_value.validate() { return false; } }
		if !self.org_id.validate() { return false }
		return true
	}
}
