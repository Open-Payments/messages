...
impl IdentificationVerificationReportV04 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Some(ref val) = self.orgnl_assgnmt { if let Err(e) = val.validate() { return Err(e); } }
...
impl VerificationReport5 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Some(ref val) = self.updtd_pty_and_acct_id { if let Err(e) = val.validate() { return Err(e); } }
		Ok(())
	}
}
