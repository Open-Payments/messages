...
pub mod iso20022 {
	...
	pub struct ExternalVerificationReason1Code {
		#[cfg_attr( feature = "derive_serde", serde(rename = "$value") )]
		pub external_verification_reason1_code: String,
	}
	
	impl ExternalVerificationReason1Code {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if self.external_verification_reason1_code.chars().count() < 1 {
			return Err(ValidationError::new(1001, "external_verification_reason1_code is shorter than the minimum length of 1".to_string()));
			}
			if self.external_verification_reason1_code.chars().count() > 4 {
				return Err(ValidationError::new(1002, "external_verification_reason1_code exceeds the maximum length of 4".to_string()));
			}
			Ok(())
		}
	}
	...
	impl IdentificationAssignment4 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Some(ref cretr_value) = self.cretr { if let Err(e) = cretr_value.validate() { return Err(e); } }
			if let Some(ref frst_agt_value) = self.frst_agt { if let Err(e) = frst_agt_value.validate() { return Err(e); } }
	...
	pub struct IdentificationVerificationIndicator {
		#[cfg_attr( feature = "derive_serde", serde(rename = "$value") )]
		pub identification_verification_indicator: bool,
	}
	
	impl IdentificationVerificationIndicator {
		pub fn validate(&self) -> Result<(), ValidationError> {
			Ok(())
		}
	}
	...
	impl IdentificationVerificationReportV04 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Some(ref orgnl_assgnmt_value) = self.orgnl_assgnmt { if let Err(e) = orgnl_assgnmt_value.validate() { return Err(e); } }
	...
	impl VerificationReport5 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			if let Err(e) = self.orgnl_id.validate() { return Err(e); }
			if let Some(ref rsn_value) = self.rsn { if let Err(e) = rsn_value.validate() { return Err(e); } }
			if let Some(ref orgnl_pty_and_acct_id_value) = self.orgnl_pty_and_acct_id { if let Err(e) = orgnl_pty_and_acct_id_value.validate() { return Err(e); } }
			if let Some(ref updtd_pty_and_acct_id_value) = self.updtd_pty_and_acct_id { if let Err(e) = updtd_pty_and_acct_id_value.validate() { return Err(e); } }
			Ok(())
		}
	}
