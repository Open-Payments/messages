...
impl IdentificationVerification5 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if let Err(e) = self.id.validate() { return Err(e); }
		if let Err(e) = self.pty_and_acct_id.validate() { return Err(e); }
		Ok(())
	}
}
...
impl IdentificationVerificationRequestV04 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		if let Err(e) = self.assgnmt.validate() { return Err(e); }
		for item in &self.vrfctn { if let Err(e) = item.validate() { return Err(e); } }
