...
impl AccountSwitchCancelExistingPaymentV05 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Some(ref pmt_instr_vec) = self.pmt_instr { for item in pmt_instr_vec { if let Err(e) = item.validate() { return Err(e); } } }
		if let Some(ref drct_dbt_instr_vec) = self.drct_dbt_instr { for item in drct_dbt_instr_vec { if let Err(e) = item.validate() { return Err(e); } } }
...
impl DirectDebitInstructionDetails3 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Some(ref last_colltn_ccy_amt_value) = self.last_colltn_ccy_amt { if let Err(e) = last_colltn_ccy_amt_value.validate() { return Err(e); } }
