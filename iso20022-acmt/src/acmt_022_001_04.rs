...
pub mod iso20022 {
	...
	impl IdentificationModification5 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Err(e) = self.updtd_pty_and_acct_id.validate() { return Err(e); }
	...
	impl IdentificationModificationAdviceV04 {
		pub fn validate(&self) -> Result<(), ValidationError> {
	...
	impl OriginalTransactionReference43 {
		pub fn validate(&self) -> Result<(), ValidationError> {
			...
			if let Some(ref orgnl_tx_vec) = self.orgnl_tx { for item in orgnl_tx_vec { if let Err(e) = item.validate() { return Err(e); } } }
			Ok(())
		}
	}
