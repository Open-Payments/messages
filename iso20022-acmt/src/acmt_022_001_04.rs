...
impl IdentificationAssignment4 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Some(ref val) = self.cretr { if let Err(e) = val.validate() { return Err(e); } }
		if let Some(ref val) = self.frst_agt { if let Err(e) = val.validate() { return Err(e); } }
...
impl IdentificationModification5 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Some(ref val) = self.orgnl_pty_and_acct_id { if let Err(e) = val.validate() { return Err(e); } }
...
impl OriginalTransactionReference43 {
	pub fn validate(&self) -> Result<(), ValidationError> {
		...
		if let Some(ref vec) = self.orgnl_tx { for item in vec { if let Err(e) = item.validate() { return Err(e); } } }
		Ok(())
	}
}
